                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-lg-4 col-md-4 col-12">
                                        <select class="form-control" name="status">
                                            ...
                                            <option value="1"
                                                {{ request()->input('status') === '1' ? 'selected' : '' }}>Đã xem
                                            </option>
                                            <option value="0"
                                                {{ request()->input('status') === '0' ? 'selected' : '' }}>Chưa xem
                                            </option>
                                        </select>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on("click", ".toggle-status", function() {
                ...
                fetch(`/admin/contact/toggle/${feelingId}`, {
                        method: 'POST',
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            button.text(data.is_approved ? 'Đã xem' : 'Chưa xem');

                            button.toggleClass("btn-success btn-danger"); // Đổi màu nút

                            toastr.success(data.is_approved ? 'Được xem.' :
                                'Bị hủy xem.');
                        }
                    })
