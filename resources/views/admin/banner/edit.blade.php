...
@section('content')
    <div class="container">
        ...
        <form class="row" action="{{ route('banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
            <div class="col-12">
                        ...
                        <div class="d-flex flex-column flex-md-row align-items-center mb-4 file-input-wrapper gap-2">
                            <div>
                                <img class="image avatar avatar-lg rounded-3"
                                    src="{{ asset($banner->image) }}" alt="Image">
                            </div>
                                ...
                                <div class="col-lg-6 col-12">
                                    <div>
                                        ...
                                        <input type="text" name="name" class="form-control" id="name"
                                            placeholder="Tiêu đề" value="{{ old('name', $banner->name) }}" />
                                        @error('name')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-lg-6 col-12">
                                    <div>
                                        ...
                                        <input type="text" name="link" class="form-control" id="link"
                                            placeholder="Đường dẫn" value="{{ old('link', $banner->link) }}"/>
                                        @error('link')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
